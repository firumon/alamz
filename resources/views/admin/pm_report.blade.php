...
@section('content')
    @if($hasDate)
        @php
            $records = recordsBetween(request('start'),request('end'))->groupBy(headValues(config('alramz.fields.pm_name')))->map(function($item){ return statusCount($item); })->toArray();
            $Total = [];
        @endphp
        <div class="table-responsive">
            <table class="table table-sm" id="report-list-table">
                @if(!empty($records))<thead><tr><th>Portfolio Manager</th>@foreach($records[array_key_first($records)] as $th => $count) <th>{{ $th }}</th>@php $Total[$th] = 0 @endphp @endforeach</tr></thead>@endif
                <tbody>
                @forelse($records as $pm => $counts)
                    <tr>
                        <td>{{ $pm }}</td>
