...
if(request()->getMethod() === 'POST'){
    if(request()->submit === 'Update Detail' && request()->filled(['id'])){
        \App\User::where('id',request()->id)->update(request()->merge(['role' => 'admin'])->only(['name','email','role','status']));
        \App\Activity::activity('Updated details of user ' . request('name'));
    } else {
        \App\User::create(request()->merge(['role' => 'admin'])->only(['name','email','role','password']));
...
@section('content')
    ...
    @section('button')<button class="btn btn-primary" onclick="AddAdmin()">Add Admin</button>@stop
