...
if(request()->getMethod() === 'POST'){
    if(request()->submit === 'Update Detail' && request()->filled(['id'])){
        \App\User::where('id',request()->id)->update(request()->merge(['role' => 'compliance'])->only(['name','email','role','status']));
        \App\Activity::activity('Updated details of user ' . request('name'));
    } else {
        \App\User::create(request()->merge(['role' => 'compliance'])->only(['name','email','role','password']));
        \App\Activity::activity('Created new user ' . request('name') . ', with role ' . request('role'));
    }
}
$users = \App\User::where('role','compliance')->get();
@endphp
...
@section('content')
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">Add new Compliance</h5>
                        ...
                        <div class="modal-footer">
                            <input type="submit" name="submit" value="Add Compliance" class="btn btn-primary">
                        </div>
                </div>
            </div>
        </div>
    ...
    @section('button')<button class="btn btn-primary" onclick="AddCompliance()">Add Compliance</button>@stop
    ...
    <div class="table-responsive">
        <table class="table table-sm">
            ...
            <tbody>
                ...
                @empty
                    <tr><td colspan="5">No Compliance found!!!</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
