                    ...
                    <form method="post" target="_blank" action="{{ route('records.preview') }}">@csrf
                        ...
                        <div class="form-row">
                            <div class="form-group col-6">
                                ...
                                <select class="form-control" multiple name="{{ HEAD_FIELD_DISPLAY['Compliance'] }}[]"><option value="" selected>All</option>@foreach(\App\User::where('role','compliance')->get() as $compliance) <option value="{{ $compliance->name }}">{{ $compliance->name }}</option> @endforeach</select>
                            </div>
